...
export default function ProductCard({ product: p, onAdd, featured = false }: Props) {
    ...
    return (
      <div className="card-featured">
        ...
        <Link href={`/products/${p.slug}`} style={{ textDecoration: "none" }}>
          <div
            style={{ position: "relative", height: "clamp(260px, 40vw, 360px)", overflow: "hidden", background: "var(--ink)", cursor: "pointer" }}
  ...
  return (
    ...
    >
      {/* Image cliquable */}
      <Link href={`/products/${p.slug}`} style={{ textDecoration: "none" }}>
        <div style={{ position: "relative", height: 190, overflow: "hidden", background: "#111", cursor: "pointer" }}>
          <img
            src={p.img}
            alt={p.name}
            style={{
